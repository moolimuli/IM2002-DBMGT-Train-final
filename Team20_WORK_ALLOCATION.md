# Work Allocation Report — [Team 20]
...
|--------|-----------|---------------------|
余妍萱 | 33% | Task 1: schema design (schema1/schema2 separation, Argon2id architecture, fixed CREATE INDEX syntax); Task 2: all queries.py functions implementation and optimization, including query_user_profile (added year_of_birth field for B6 grading requirement); Task 2d: Argon2id password hashing and verify_password.py script; Bug fixes: O1 (execute_cancellation departure_time fix), agent.py skip logic (skip only truly empty required parameters); agent.py tool descriptions optimization; end-to-end testing of all 9 instructor test questions; Task 6: UI extension development; Design Doc Section 1 (ER diagram), Section 5 (AI evidence), Section 7.5 (UI enhancement design)
